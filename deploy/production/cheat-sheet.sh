...
helm install cockroach-auth -f cockroach.values.yaml --namespace=${PROJECT_NAME} cockroachdb/cockroachdb --version 6.0.9
kubectl -n=${PROJECT_NAME} get pv -w
kubectl -n=${PROJECT_NAME} get pv datadir-cockroach-auth-cockroachdb-0 datadir-cockroach-auth-cockroachdb-1 datadir-cockroach-auth-cockroachdb-2
